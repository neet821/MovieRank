...
async function loadMovieRanks() {
    ...
    try {
        const response = await fetch("/movie-ranks");
        if (!response.ok) {
            throw new Error(`HTTP ${response.status}`);
        }
        ...
        tbody.innerHTML = `<tr><td colspan="8">电影排名加载失败：${error.message}</td></tr>`;
    }
}
...
async function submitLogin(event) {
    ...
    const resultBox = document.querySelector("#loginResult");
    const username = document.querySelector("#username").value.trim();
    const password = document.querySelector("#password").value;

    resultBox.textContent = "正在请求 /login ...";
    ...
    try {
        ...
        resultBox.textContent = `登录请求失败：${error.message}`;
    }
}
