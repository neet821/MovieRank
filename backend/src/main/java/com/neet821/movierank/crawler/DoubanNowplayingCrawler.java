...
public class DoubanNowplayingCrawler {
  ...
  private static final String NOWPLAYING_URL =
      "https://movie.douban.com/cinema/nowplaying/wuhan/";
  private static final Logger log =
      LoggerFactory.getLogger(DoubanNowplayingCrawler.class);
  ...
  public List<NowPlayingMovie> parse(Document doc) {
    ...
    log.info("Selector '#nowplaying .lists > li.list-item' matched {} items",
             items.size());
    if (items.isEmpty()) {
      items = doc.select("#nowplaying li.list-item");
      log.info("Selector '#nowplaying li.list-item' matched {} items",
               items.size());
    }
    if (items.isEmpty()) {
    ...
    for (Element item : items) {
      ...
      // 电影名 — 优先用 data-title 属性（完整名），否则用 .stitle a 的 title
      // 属性
      ...
      if (posterImg != null) {
        ...
        if (posterUrl.isBlank()) {
          posterUrl = posterImg.attr("data-src");
        }
      }

      movie.setPosterUrl(buildPosterUrl(title, posterUrl));
  ...
  private String buildPosterUrl(String title, String doubanPosterUrl) {
    if (title == null || title.isBlank()) {
      return null;
    }
    StringBuilder url =
        new StringBuilder("/posters/nowplaying?title=")
            .append(URLEncoder.encode(title, StandardCharsets.UTF_8));
    if (doubanPosterUrl != null && !doubanPosterUrl.isBlank()) {
      url.append("&doubanUrl=")
          .append(URLEncoder.encode(doubanPosterUrl, StandardCharsets.UTF_8));
    }
    return url.toString();
  }
}
